...
class UNIFIED_PAYMENT_GATEWAY extends WC_Payment_Gateway_CC
{
		...
		if (empty($errors)) {
			...
			if (class_exists('UNIFIED_PAYMENT_GATEWAY_Loader')) {
				...
				if (method_exists($loader, 'handle_cron_event')) {
					...
					$this->log_info('Triggered UNIFIED_PAYMENT_GATEWAY_Loader::handle_cron_event() after settings save.');
				}
			}
		} else {
	...
	public function process_payment($order_id, $used_accounts = []) {
		...
		if (!$order) {
			...
			if (is_checkout()) wc_add_notice(__('Invalid order.', 'unified-payment-gateway'), 'error');
			return ['result' => 'fail'];
		}
		...
		if (!filter_var($billing_email, FILTER_VALIDATE_EMAIL)) {
			if (is_checkout()) wc_add_notice(__('Please enter a valid email address.', 'unified-payment-gateway'), 'error');
		...
		if ($this->is_po_box($billing)) {
			if (is_checkout()) wc_add_notice(__('PO Box addresses are not allowed.', 'unified-payment-gateway'), 'error');
		...
		if (count($timestamps) >= $max_requests) {
			...
			if (is_checkout()) wc_add_notice(__('Too many requests. Please try again later.', 'unified-payment-gateway'), 'error');
			return ['result' => 'fail'];
		}
		...
		if ($this->sandbox) {
			if (!$order->get_meta('_is_test_order')) {
				...
				$order->add_order_note(__('This is a test order processed in sandbox mode.', 'unified-payment-gateway'));
				$order->save();
			}
		}
		...
		if (!$selected_account) {
			if ($last_error_data) {
				if (isset($last_error_data['max_limit_reached']) && $last_error_data['max_limit_reached'] == true) {
					wc_add_notice(__('The transaction amount exceeds the maximum allowed limit of '.$last_error_data['max_amount'].'. Please enter a lower amount.', 'unified-payment-gateway'), 'error');
					return ['result' => 'fail'];
				}
			...
			$raw = get_option('woocommerce_unified_payment_gateway_accounts', []);
			...
			if (is_checkout()) wc_add_notice(__('No available payment accounts.', 'unified-payment-gateway'), 'error');
		...
		if (is_wp_error($response)) {
			...
			if (is_checkout()) wc_add_notice(__('Payment error: Unable to process.', 'unified-payment-gateway'), 'error');
			return ['result' => 'fail'];
		}
		...
		// Success
		if (!empty($resp_data['data']['payment_link'])) {
			if ($pay_id) $order->update_meta_data('_unified_pay_id', $pay_id);
			$order->update_status('pending', __('Payment pending.', 'unified-payment-gateway'));
			$order->add_order_note(sprintf(__('Payment initiated via Unified. Awaiting completion (%s)', 'unified-payment-gateway'), $account['title']));
			$order->save();
		...
		if (is_checkout()) wc_add_notice(__('Payment failed. Please try again.', 'unified-payment-gateway'), 'error');
		return ['result' => 'fail'];
	}
	...
	private function unified_prepare_payment_data($order, $api_public_key, $api_secret) {
		...
		$normalized  = $this->unified_normalize_phone($phone_for_normalization, $country_code);
	...
	public function unified_hide_custom_payment_gateway_conditionally($available_gateways) {
		...
		$this->log_info_once_per_session('gateway_check_start_' . $cart_hash, 'Checking Unified payment option availability', ['cart_hash' => $cart_hash]);
		...
		if ($all_accounts_limited) {
			 $this->log_info_once_per_session('accounts_limited_' . $cart_hash, 'Unified payment option hidden: all accounts have reached their transaction limits');
		...
		if (!empty($selected_account['checkout_title'])) {
			...
			//print_r($available_gateways['unified']);exit;				
...
private function get_routing_sorted_accounts(array $accounts): array {
	...
	private function get_next_available_account($used_accounts = []){
		...
		$account['lock_key'] = "unified_lock_{$sanitized_title}";
			...
			if (is_string($value)) {
				foreach ($sql_injection_patterns as $pattern) {
					if (preg_match($pattern, $value)) {
						...
						wc_get_logger()->info("Potential SQL Injection - Field: $field_label, IP: {$ip_address}", ['source' => 'unified-payment-gateway']);
