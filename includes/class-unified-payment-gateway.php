...
class UNIFIED_PAYMENT_GATEWAY extends WC_Payment_Gateway_CC
{
	...
	public function __construct() {
		...
		$this->method_description = __('This plugin allows you to accept payments in USD through a secure payment gateway integration. Customers can complete their payment process with ease and security.', 'unified-payment-gateway');
	    ...
	    foreach ((array) $raw_accounts as $account) {
	        ...
	        if ($has_sandbox && !empty($sandbox_public_key) && !empty($sandbox_secret_key)) {
	           ...
	           if ($sandbox_public_key === $sandbox_secret_key) {
					...
					$errors[] = sprintf(
						/* translators: %s: The account title entered by the user. */
						__('Account "%s": Sandbox Public Key and Sandbox Secret Key must be different.', 'unified-payment-gateway'),
						$account_title
					);
	...
	public function unified_get_form_fields() {
	    ...
	    return apply_filters('unified_woocommerce_gateway_settings_fields_' . $this->id, [
			...
			'description' => [
			    ...
			    'default'     => __(
			        '<p style="margin:0 0 6px; font-size:13px;">On your bank or card statement, this transaction will appear as from *Unified.</p>
...
<?php return ob_get_clean();
	...
	public function process_payment($order_id, $used_accounts = [])
	{
		...
		if ($this->is_po_box($billing)) {
			if ( is_checkout() ) {
				wc_add_notice(__('PO Box addresses are not allowed.', 'unified-payment-gateway'), 'error');
			}
		...
		// --------------------------
		if ($this->sandbox) {
			$note_text = __('This is a test order processed in sandbox mode.', 'unified-payment-gateway');
		...
		while (true) {
			...
			wc_get_logger()->info("Payment API raw response", ['source'=>'unified-payment-gateway','context'=>$resp_data]);
			...
			// Handle Failure
			// --------------------------
			$error_msg = sanitize_text_field($resp_data['message'] ?? __('Payment failed.','unified-payment-gateway'));
			...
			if ( is_checkout() ) {
				wc_add_notice(__('Payment error: ', 'unified-payment-gateway').$error_msg,'error');
			}
	...
	public function unified_hide_custom_payment_gateway_conditionally($available_gateways)
	{
		...
		$cache_key = 'unified_gateway_visibility_' . $gateway_id . '_' . $cart_hash;
		...
		$this->log_info_once_per_session(
			...
			'Checking Unified payment option availability',
			['cart_hash' => $cart_hash]
		);
		...
		if ($amount < 0.01) {
			$this->log_info_once_per_session(
				...
				'Unified payment option hidden: order total is too low',
				['Amount' => $amount]
			);
			$this->log_info_once_per_session(
				'gateway_check_end_' . $cart_hash,
				'Unified payment option check finished'
			);
		...
		if (!method_exists($this, 'get_all_accounts')) {
			$this->log_info_once_per_session(
				...
				'Unified plugin misconfigured: account retrieval method missing'
			);
			$this->log_info_once_per_session(
				'gateway_check_end_' . $cart_hash,
				'Unified payment option check finished'
		...
		$this->log_info_once_per_session(
			...
			'Unified account status summary',
			[
		...
		if (empty($accounts)) {
			$this->log_info_once_per_session(
				...
				'Unified payment option hidden: no accounts configured'
			);
			$this->log_info_once_per_session(
				'gateway_check_end_' . $cart_hash,
				'Unified payment option check finished'
		...
		if (!$user_account_active) {
			$this->log_info_once_per_session(
				...
				'Unified payment option hidden: no active accounts'
			);
			$this->log_info_once_per_session(
				'gateway_check_end_' . $cart_hash,
				'Unified payment option check finished'
		...
		if ($all_accounts_limited) {
			$this->log_info_once_per_session(
				...
				'Unified payment option hidden: all accounts have reached their transaction limits'
			);
			$this->log_info_once_per_session(
				'gateway_check_end_' . $cart_hash,
				'Unified payment option check finished'
		...
		$this->log_info_once_per_session(
			...
			'Unified payment option is visible: account is active'
		);
		$this->log_info_once_per_session(
			'gateway_check_end_' . $cart_hash,
			'Unified payment option check finished'
	...
	{
		$btyenftApiUrl = $this->get_api_url('/api/switch-account-email'); // Unified API Endpoint
	...
	private function get_next_available_account($used_accounts = [])
    {
        ...
        foreach ($available_accounts as $account) {
            ...
            $lock_key = "unified_lock_{$sanitized_title}";
	...
	private function acquire_lock($lock_key)
	{
	    ...
	    if ($existing_lock && intval($existing_lock) > $now) {
	        ...
	        wc_get_logger()->info("Lock already held for '{$lock_key}', skipping.", ['source' => 'unified-payment-gateway']);
	        return false;
	    }
