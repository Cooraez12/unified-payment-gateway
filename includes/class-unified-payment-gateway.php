...
<?php return ob_get_clean();
	...
	public function process_payment($order_id, $used_accounts = [])
	{
		...
		while (true) {
			...
			if (is_array($data) && isset($data['result']) && $data['result'] === 'fail') {
				// Validation error already logged and notice added in unified_prepare_payment_data
			...
			$error_message = isset($response_data['message']) ? sanitize_text_field($response_data['message']) : __('Payment failed.', 'unified-payment-gateway');
	...
	private function unified_prepare_payment_data($order, $api_public_key, $api_secret)
	{
		...
		if (empty($normalized['is_valid'])) {
			$error_message = $normalized['error'] ?? __('Phone number validation failed.', 'unified-payment-gateway');
...
private function unified_normalize_phone($phone, $country_code)
{
	...
	public function unified_hide_custom_payment_gateway_conditionally($available_gateways)
	{
	    ...
	    $cache_key = 'unified_gateway_visibility_' . $gateway_id;
