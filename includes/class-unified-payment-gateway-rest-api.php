	...
	public function unified_handle_api_request(WP_REST_Request $request)
	{
		...
		if ($api_order_status === 'completed') {
		    ...
		    if (!in_array($current_order_status, ['completed'])) {
		        ...
		        $this->logger->info("Order {$order_id} is already in '{$current_order_status}' status. No change for duplicate 'failed' status.", ['source' => 'unified-payment-gateway']);
		    ...
		    if (!in_array($current_order_status, ['cancelled', 'completed'])) {
		        ...
		        $this->logger->info("Order {$order_id} is already in '{$current_order_status}' status. No change for duplicate 'cancelled' status.", ['source' => 'unified-payment-gateway']);
